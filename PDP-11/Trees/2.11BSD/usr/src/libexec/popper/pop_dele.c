...
POP     *   p;
{
    ...
    if ((msg_num < 1) || (msg_num > p->msg_count))
        return (pop_msg (p,POP_FAILURE,"Message %u does not exist.",msg_num));
    ...
    if (mp->del_flag)
        return (pop_msg (p,POP_FAILURE,"Message %u has already been deleted.",
            msg_num));
...
#ifdef DEBUG
    if(p->debug)
        pop_log(p,POP_DEBUG,"Deleting message %u at offset %ld of length %ld\n",
    ...
    return (pop_msg (p,POP_SUCCESS,"Message %u has been deleted.",msg_num));
}
