    ...
    if (p->parm_count > 0) {
        ...
        if ((msg_num < 1) || (msg_num > p->msg_count))
            return (pop_msg (p,POP_FAILURE,
                "Message %u does not exist.",msg_num));
        ...
        if (mp->del_flag)
            return (pop_msg (p,POP_FAILURE,
                "Message %u has been deleted.",msg_num));

        /*  Display message information */
        return (pop_msg(p,POP_SUCCESS,"%u %ld",msg_num,mp->length));
    }
    
    /*  Display the entire list of messages */
    pop_msg(p,POP_SUCCESS,
        "%u messages (%ld octets)",
