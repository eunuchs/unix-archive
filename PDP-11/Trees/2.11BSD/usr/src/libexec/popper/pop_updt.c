...
POP     *   p;
{
    ...
    if (p->msgs_deleted == p->msg_count) {
        ...
        (void)ftruncate ((int)fileno(p->drop),0L);
        (void)fclose(p->drop) ;
        return (POP_SUCCESS);
    }
    ...
    offset = lseek((int)fileno(p->drop),0L,L_XTND) ; 
    ...
    rewind(p->drop);
    (void)lseek((int)fileno(p->drop),0L,L_SET);
    ...
    for (msg_num = 0; msg_num < p->msg_count; ++msg_num) {
        ...
        if (mp->del_flag) {
            ...
            if(p->debug)
                pop_log(p,POP_DEBUG,
                    "Message %u flagged for deletion.",mp->number);
#endif DEBUG
            continue;
        }
...
#ifdef DEBUG
        if(p->debug)
            pop_log(p,POP_DEBUG,"Copying message %u.",mp->number);
#endif DEBUG
