...
#define	dkbad	dkbbuf.bad
union	dkbbuf oldbuf;
#define	oldbad	oldbuf.bad
	...
	char *argv[];
{
	...
	size = (daddr_t)dp->d_nsectors * dp->d_ntracks * dp->d_ncylinders; 
#ifdef	HACK
printf("name \"%s\" type \"%s\" size %ld\n", dp->d_name, dp->d_type, size);
printf("secsize %d ntracks %d nsectors %d ncylinders %d\n",
	dp->d_secsize, dp->d_ntracks, dp->d_nsectors, dp->d_ncylinders);
printf("rpm %d badsectforw %d sectoffset %d\n",
	dp->d_rpm, dp->d_badsectforw, dp->d_sectoffset);
for(i=0; i<8; i++) {
	printf(" %s%c size %ld bsize %d fsize %d\n",
		argv[1], "abcdefgh"[i],
		dp->d_partitions[i].p_size,
		dp->d_partitions[i].p_bsize,
		dp->d_partitions[i].p_fsize);
}
exit(0);
#endif
	if (argv[1][0] != '/')
	...
	if (add) {
		...
		if (i + argc > maxbad) {
			printf("bad144: system not configured for %d more sectors\n",
				argc);
			printf("limited to %d by current configuration\n", maxbad);
		} else if (i + argc > 126) {
	...
	new = argc;
	while (argc > 0) {
		daddr_t sn = atol(*argv++);
		argc--;
		if (sn < 0 || sn >= size) {
			printf("%ld: out of range [0,%ld) for %s\n",
			    sn, size, dp->d_name);
			errs++;
			continue;
		}
...
union dkbbuf *bad;
{
	...
	for (i = 0; i < 10 && i < dp->d_nsectors; i += 2) {
		...
		if (lseek(f, (off_t)sn * dp->d_secsize, L_SET) < 0)
			Perror("lseek");
		if (read(f, (char *)bad, 512) == 512) {
			if (i > 0)
				printf("Using bad-sector file %d\n", i/2);
			return(sn);
		}
		(void)sprintf(msg, "bad144: read bad sector file at sn %ld",
		    sn);
		perror(msg);
	}
	fprintf(stderr,
...
register struct bt_bad *bt;
{
	return (((daddr_t)bt->bt_cyl*dp->d_ntracks + (bt->bt_trksec>>8)) * dp->d_nsectors
		+ (bt->bt_trksec&0xff));
}
	...
	daddr_t blk;
{
	 ...
	 */
	if (lseek(fd, (off_t)blk * dp->d_secsize, L_SET) < 0)
		Perror("lseek");
	if (verbose)
		printf("format blk %ld\n", blk);
	if (ioctl(fd, DKIOCHDR, (char *)0) < 0)
		Perror("ioctl DKIOCHDR 1");
