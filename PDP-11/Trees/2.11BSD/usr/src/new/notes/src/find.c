...
struct note_f  *note;
{
    ...
    dprintf ("wantlen %d, wantdomain %s\n", wantlen, wantdomain);
#endif	IDCOMPAT

    for (i = io -> descr.d_nnote; i > 0; i--)		/* dups likely "new" */
    {							/* so search backwards */
	getnrec (io, i, note);
	dprintf("noteid->sys %s note->n_id.sys %s\n",
	    noteid->sys, note->n_id.sys);
	...
	if (noteid -> uniqid == note -> n_id.uniqid)	/* cheap to do */
	{
	    ...
	    dprintf ("thislen %d, thisdomain %s, shortlen %d\n",
		thislen, thisdomain, shortlen);
