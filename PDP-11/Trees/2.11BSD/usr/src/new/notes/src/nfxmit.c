...
#ifdef	RCSIDENT
static char rcsid[] = "$Header: nfxmit.c,v 1.7.0.1 86/01/17 21:15:38 notes Rel $";
#endif	RCSIDENT
...
char   *local;
{
    ...
    struct nethow_f how;				/* how to send */

    getnet (tosite, local, &how);			/* see if non standard */
    nfalias (local, nfname, tosite);			/* get remote file name */

    if (callback)
    {
	if (how.nh_rcv == (char *) NULL)
	    sprintf (cmdline, DFLTRPLY,			/* see net.h defn */
		    tosite, NFXMIT, nfname, System);
	else
	    sprintf (cmdline, how.nh_rcv, nfname, System);/* do his */
#ifndef	FASTFORK
 ...
 */
    ...
    if ((sendstat =					/* WANT ASSIGN */
		nfsend (tosite, local, dmpfile, usetime, sendhim, how.nh_proto)) > 0)
    {
	if (how.nh_xmit == (char *) NULL)
	    sprintf (cmdline, DFLTXMIT,			/* see net.h defn */
		    tosite, NFRCV, nfname, System, dmpfile);
	else
	{
	    sprintf (buf, how.nh_xmit, nfname, System);
 ...
 */
    ...
    if (how.nh_system != (char *) NULL)			/* free storage */
	free (how.nh_system);
    if (how.nh_nf != (char *) NULL)
	free (how.nh_nf);
    if (how.nh_xmit != (char *) NULL)
	free (how.nh_xmit);
    if (how.nh_rcv != (char *) NULL)
