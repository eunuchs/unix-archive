...
#ifdef	RCSIDENT
static char rcsid[] = "$Header: archiver.c,v 1.7.0.6 86/01/25 22:54:08 notes Rel $";
#endif	RCSIDENT
 ...
 */
    ...
    {
	getnrec (&io, i, &note);
	if (note.n_stat & DELETED)			/* already gone */
	    continue;
	if (inorder (&zaptime, &note.n_lmod))		/* check times */
	    continue;					/* too recent */
	if (dirmsgflag == DIROFF && (note.n_stat & DIRMES))
	    continue;					/* only dir off */
	if (dirmsgflag == DIRON && (note.n_stat & DIRMES) == 0)
	    continue;					/* only dir on */
	if (dirmsgflag == DIRANYON)			/* any w/dir on */
	{
	    /* 
	     * if anything in the note string has dir message on,
	     * expire the sucker.
	     */
	    if (note.n_stat & DIRMES)			/* note has dir */
		goto killit;				/* so expire it */
	    for (rnum = 1; rnum <= note.n_nresp; rnum++)
	    {						/* for each response */
		if (lrsp (&io, i, rnum, &resp, &roffset, &rblock) != 0)
		    break;				/* busted resp chain */
		if (resp.r_stat[roffset] & DIRMES)	/* is dir message */
		    goto killit;			/* so expire */
	    }
