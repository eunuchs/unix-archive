...
#ifdef	RCSIDENT
static char rcsid[] = "$Header: parsepath.c,v 1.7.0.3 85/11/29 12:18:43 notes Rel $";
#endif	RCSIDENT
...
char   *fromline;
{
	...
	if ((p = index (line, '<')) != (char *) NULL)	/* get it */
	{
		...
		goto usepath;
	    *q = '\0';					/* truncate */
	    strcpy (strippedline, ++p);			/* and save */
	}
	else
	{
	    int     incomment;
	    ...
	    while (*q != '\0')
	    {
		if (incomment > 0 && *q == ')')		/* end comment */
		{
		    incomment--;
		    q++;
		    continue;
		}
		if (incomment > 0)			/* ignore char */
		{
		    q++;
		    continue;
		}
	...
	if ((p = index (strippedline, '@')) == (char *) NULL)
	    goto usepath;
	*p++ = '\0';					/* split them */
	strncpy (authname, strippedline, NAMESZ);
	 ...
	 */
	if (strlen (authname) > 0)			/* could be null */
