 ...
 *
 * Modified March 1978 to hash old messages to be able to recomp_
 ...
 *
 * The program looks for 'xprintf("' in the source stream.
...
copystr()
{
	...
	for (;;) {
		...
		*obufp++ = c; /* save copy */
		...
		case '"':
			/* jpn: skip it all if string < 6 chars */
			if ( (obufp - obuf) < 6 ) {
			    *obufp = 0;
			    printf("xprintf(\"%s", obuf);
			    return;
			}
			*cp++ = 0;
			goto out;
...
out:
	*cp = 0;
	printf("mkprintf(%d", hashit(buf, 1, NULL));
	return;
}
